import{z as B,r as m,j as e,ac as M,aV as C,a_ as L,av as E,aI as U}from"./vendor-DjsjSLNV.js";import{a as y,p as k,f as i,e as g}from"./formatters-CfNdLP9c.js";import{c as z}from"./corporatePrintHeaderHtml-C-paRYIb.js";import{a as t}from"./AdminUi-BGDWg3Zk.js";import{S as R}from"./index-BM_gXBrB.js";function $(a){return`INV-${String(a?.id||"").padStart(6,"0")}`}function F(a){return a?.reference_number||a?.reference_no||`PAY-${a?.id||"N/A"}`}function I(a,s){return a?.borrower_name||a?.borrower?.name||s?.name||s?.full_name||"Borrower"}function S(a){return a?.processed_by_name||a?.encoder_name||a?.recorded_by_user?.name||a?.confirmed_by_user?.name||""}function v(a){return String(a?.processed_by_role||a?.encoder_role||a?.receipt_issued_role||"").trim().replace(/[_-]+/g," ").replace(/\b\w/g,n=>n.toUpperCase())}function _(a){return String(a?.official_receipt_number||a?.or_number||"").trim()}function A(a){return String(a?.acknowledgement_receipt_number||a?.ar_number||"").trim()}function D(a,s){const n=$(a),o=y(a?.paid_at||a?.due_date),d=i(a?.amount_paid||0),l=i(a?.amount_due||0),w=i(a?.penalty_amount||0),x=F(a),f=I(a,s),p=s?.email||a?.borrower_email||"N/A",u=_(a),j=A(a),h=S(a)||"Authorized representative",r=v(a),c=typeof window<"u"?new URL("/amalgated-lending-logo.png",window.location.origin).toString():"/amalgated-lending-logo.png";return`<!doctype html>
<html>
<head>
  <meta charset="utf-8" />
  <title>${n}</title>
