...
export default function LoanEvaluationPanel({
  ...
  useEffect(() => {
    ...
    setProposedLoanAmount(numOrEmpty(applicationLoanAmount))
  ...
  }, [savedApprovedAmount, applicationLoanAmount, savedApprovalNotes, realEstateDetail])
  ...
  const save = async () => {
    ...
    if (approvedPrincipal !== '' && !hasApproved) {
    ...
    try {
      if (hasProposed) {
        await api(`/loans/${loanId}/application-loan-amount`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ loan_amount: parsedProposed }),
        })
      }
      ...
      if (hasApproved) {
        await api(`/loans/${loanId}/approved-amount`, {
          ...
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            approved_principal: parsedApproved,
